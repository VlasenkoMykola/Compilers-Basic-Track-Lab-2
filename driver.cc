...
int main(int argc, char **argv) {
  ...
  ("eval", "evaluate AST tree (simple tiger calculator)")
  ...
  if (vm.count("eval")) {
    if (vm.count("dump-ast")) {
      utils::error("ERROR: cannot use eval and dump at the same time");
    } else {
	ast::ASTEval dumper(&std::cout, vm.count("verbose") > 0);
	parser_driver.result_ast->accept(dumper);
	dumper.nl();
    }
  }
